...
export async function verifySpendZkProof(input) {
  ...
  const orderCheck = verifyPublicInputOrder(registryEntry.entry.publicInputOrder);
  if (!orderCheck) {
  ...
  const bindingCheck = verifyBindings({ proof, spendToken });
  if (!bindingCheck.ok) {
...
function validateProofShape(proof) {
  ...
  const publicInputCheck = validateOpenMinPublicInputs(proof);
  if (!publicInputCheck.ok) {
    return publicInputCheck;
  }
...
function verifyPublicInputOrder(publicInputOrder) {
  return (
    Array.isArray(publicInputOrder) &&
    publicInputOrder.length === H2_PROMO_OPEN_MIN_V1_PUBLIC_INPUT_ORDER.length &&
    publicInputOrder.every((field, index) => field === H2_PROMO_OPEN_MIN_V1_PUBLIC_INPUT_ORDER[index])
  );
}

function verifyBindings({ proof, spendToken }) {
